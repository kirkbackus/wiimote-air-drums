...
class Media {
  ...
  static bool mciOpen(LPTSTR fname, LPTSTR alias);
  static bool mciPlay(bool restart);
  static bool mciPlay(bool restart, LPTSTR alias);
  ...
  static bool setVolume(float vol, LPTSTR alias);
...
bool Media::mciOpen(LPTSTR fname, LPTSTR alias) {
  TCHAR buffer[256];

  std::string fn;
  fn = fname;

  sprintf(buffer,TEXT("open \"%s\" alias %s"),fname,alias);
...
bool Media::mciPlay(bool restart, LPTSTR alias) {
  TCHAR buffer[128];

  // Form the command string.
  if (restart)sprintf(buffer,TEXT("play %s from 0"),alias);
  else sprintf(buffer,TEXT("play %s"),alias);
...
int Media::mciGetLength() {
  ...
  if (sreturn=atoi(((std::string)mciExecute("status music length",1)).c_str())) return sreturn;
  else return 0;
}
...
int Media::mciGetPosition() {
  ...
  if (sreturn=atoi(((std::string)mciExecute("status music position",1)).c_str())) return sreturn;
  else return 0;
}
...
bool Media::setVolume(float vol, LPTSTR alias) {
  TCHAR buffer[128];

  // Form the command string.
  sprintf(buffer,TEXT("setaudio alias %s volume to %u"), alias, (int)(vol*10));
  if (mciExecute(buffer)) {
    return 1;
  } else return 0;
}
...
float Media::getVolume() {
  ...
  if (sreturn=atoi(((std::string)mciExecute("status music volume",1)).c_str())/10) return sreturn;
  else return 0;
}
...
int Media::mciGetRecordLevel() {
  ...
  if (sreturn=atoi(((std::string)mciExecute("status raudio level",1)).c_str())) return sreturn;
  else return 0;
}
